...
interface SignupScreenProps {
  onSignupSuccess: (accessToken: string, userEmail: string, isFirstTime: boolean) => void;
...
export const SignupScreen: React.FC<SignupScreenProps> = ({
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (!projectId || !publicAnonKey) {
      console.log('Missing project configuration for signup');
    ...
    try {
      ...
      if (!signupResponse.ok) {
        ...
        if (errorText.includes('already') || errorText.includes('exists') || signupResponse.status === 422) {
      ...
      // Signup successful - Sign in the new user
      ...
      if (newSignInData.session?.access_token) {
        onSignupSuccess(
          newSignInData.session.access_token,
          newSignInData.user?.email || email,
          true
        );
      }
    } catch (err: any) {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 p-4">
        ...
        <div className="relative w-full">
          ...
          <CardContent>
            ...
            <form onSubmit={handleSignup} className="space-y-4">
              ...
              <div className="space-y-2">
                <div className="relative">
                  ...
                  <CustomButton
                    ...
                    className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 focus:outline-none cursor-pointer"
                  >
                    {showConfirmPassword ? <Eye className="w-[17px] h-[17px]" /> : <EyeOff className="w-[17px] h-[17px]" />}
                  </CustomButton>
                </div>
              </div>
              ...
              >
                {isLoading ? <LoadingDots text="Signing up" /> : 'Continue'}
              </Button>
