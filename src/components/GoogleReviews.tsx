...
import { ChevronLeft, ChevronDown, Star, Info, Brain, X, Check } from 'lucide-react';
...
export function GoogleReviews({ onBack, onSave }: GoogleReviewsProps) {
  ...
  const [initialState] = useState(() => {
    ...
    try {
      const saved = localStorage.getItem('googleReviewsSettings');
  ...
  const messageVariations = [
    "Hi {name}, thanks for choosing {business}! We'd love to hear your feedback — mind leaving a quick Google review? {link}",
    "Hey {name}, hope you had a great experience at {business}! A Google review would mean a lot to us: {link}",
    "Hi {name}, thank you for visiting {business}! If you have a moment, we'd really appreciate a review: {link}",
    "Hi {name}, we hope you enjoyed your time at {business}. Your feedback helps us improve! Leave a review here: {link}",
    "Hey {name}! Thanks for being a valued customer of {business}. Share your experience on Google: {link}",
    "Hi {name}, it was great having you at {business}! Would you mind sharing your thoughts? {link}",
    "Thank you {name} for choosing {business}! A quick review helps others find us too: {link}",
    "Hi {name}, we truly appreciate your visit to {business}. Help others discover us with a review: {link}",
  ];

  const handleGenerateMessage = async () => {
    if (isGeneratingMessage) return;
    setIsGeneratingMessage(true);
    toast.info('Generating AI review message...');
    try {
      const { companyName, lineOfBusiness } = getBusinessContext();
      const systemPrompt = `You are a business messaging assistant. Generate a short, friendly Google review request message. Use {name}, {business}, and {link} as placeholders. Keep it under 2 sentences.`;
      const userPrompt = `Generate a Google review request message${companyName ? ` for ${companyName}` : ''}${lineOfBusiness ? ` (${lineOfBusiness})` : ''}. Use {name}, {business}, and {link} as placeholders.`;
  ...
  }, [isMounted, isEnabled, reviewLink, startTiming, customStartValue, customStartUnit, stopAfterEnabled, stopAfterValue, stopAfterUnit, oneReviewPerCustomer, messageTemplate, enableReminder, reminderDays, onePerPlatform, linkEnabled, timeframeEnabled, distributionEnabled, messageEnabled, reminderEnabled]);
  ...
  const handleDiscard = () => { if (initialState) { localStorage.setItem('googleReviewsSettings', JSON.stringify(initialState)); } setShowUnsavedChanges(false); onBack(); };
              ...
              <div className="flex items-start gap-3 flex-1 min-w-0">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    <span className={`text-sm font-medium transition-colors duration-300 ${isEnabled ? 'text-gray-900 dark:text-gray-100' : 'text-gray-500 dark:text-gray-400'}`}>Activate Google Reviews</span>
                    <div className="relative group" onClick={(e) => e.stopPropagation()}>
                      ...
                      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 bg-gray-900 dark:bg-gray-700 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap z-50 shadow-lg">
                        Send automated Google review requests after appointments.
                ...
                <div className="rounded-2xl border border-gray-200 dark:border-gray-700 p-4 space-y-3 bg-white dark:bg-gray-800/50 relative z-10">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Paste your Google Maps review link</p>
                    </div>
                  ...
                  <div className="border-l-2 border-blue-400 dark:border-blue-600 pl-4 mt-3 py-1">
                    <Input type="url" value={reviewLink} onChange={(e) => setReviewLink(e.target.value)} placeholder="https://g.page/r/..." className="text-sm" />
                  </div>
                </div>
