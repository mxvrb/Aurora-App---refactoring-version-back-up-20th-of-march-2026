...
export function TrustPilotReviews({ onBack, onSave }: TrustPilotReviewsProps) {
  ...
  const [initialState] = useState(() => {
    ...
    try {
      const saved = localStorage.getItem('trustpilotReviewsSettings');
  ...
  const handleGenerateMessage = async () => {
    ...
    try {
      ...
      const systemPrompt = `You are a business messaging assistant. Generate a short, friendly TrustPilot review request message. Use {name}, {business}, and {link} as placeholders. Keep it under 2 sentences.`;
      const userPrompt = `Generate a TrustPilot review request message${companyName ? ` for ${companyName}` : ''}${lineOfBusiness ? ` (${lineOfBusiness})` : ''}. Use {name}, {business}, and {link} as placeholders.`;
  ...
  useEffect(() => {
    ...
    localStorage.setItem('trustpilotReviewsSettings', JSON.stringify(settings));
  ...
  const handleDiscard = () => { if (initialState) { localStorage.setItem('trustpilotReviewsSettings', JSON.stringify(initialState)); } setShowUnsavedChanges(false); onBack(); };
              ...
              <div className="flex items-start gap-3 flex-1 min-w-0">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    <span className={`text-sm font-medium transition-colors duration-300 ${isEnabled ? 'text-gray-900 dark:text-gray-100' : 'text-gray-500 dark:text-gray-400'}`}>Activate TrustPilot Reviews</span>
                    <div className="relative group" onClick={(e) => e.stopPropagation()}>
                      ...
                      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 bg-gray-900 dark:bg-gray-700 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap z-50 shadow-lg">
                        Send automated TrustPilot review requests after appointments.
                ...
                <div className="rounded-2xl border border-gray-200 dark:border-gray-700 p-4 space-y-3 bg-white dark:bg-gray-800/50 relative z-10">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Paste your TrustPilot review link</p>
                    </div>
                  ...
                  <div className="border-l-2 border-blue-400 dark:border-blue-600 pl-4 mt-3 py-1">
                    <Input type="url" value={reviewLink} onChange={(e) => setReviewLink(e.target.value)} placeholder="https://www.trustpilot.com/..." className="text-sm" />
                  </div>
                </div>
