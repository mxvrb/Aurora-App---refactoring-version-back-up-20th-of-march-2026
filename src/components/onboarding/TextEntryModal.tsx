...
import { Plus, Trash2, Bold, Italic, Strikethrough, List, Link as LinkIcon, Smile, Type, FileText, ChevronDown } from 'lucide-react';
import { Popover, PopoverContent, PopoverTrigger } from '../ui/popover';
import EmojiPicker from 'emoji-picker-react';
import Vector118920 from '../../imports/Vector-1189-20';
...
interface TextEntryModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (snippets: { title: string; text: string }[]) => void;
  initialSnippets?: { title: string; text: string }[];
}
...
export const TextEntryModal: React.FC<TextEntryModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      const formatted = initialSnippets.map((s, i) => ({
        id: `snippet-${i}-${Date.now()}`,
        title: s.title,
        text: s.text
      }));
      setSnippets(formatted);
      if (formatted.length > 0) {
        selectSnippet(formatted[0]);
      } else {
        createNewSnippet();
      }
    }
  ...
  const selectSnippet = (snippet: TextSnippet) => {
    setSelectedId(snippet.id);
    setCurrentTitle(snippet.title);
    setTimeout(() => {
      if (editorRef.current) {
        editorRef.current.innerHTML = snippet.text;
      }
    }, 0);
  };
  ...
  const createNewSnippet = () => {
    ...
    setTimeout(() => selectSnippet(newSnippet), 0);
  };

  const updateCurrentSnippet = (updates: Partial<TextSnippet>) => {
  ...
  const execCommand = (command: string, value: string | undefined = undefined) => {
    ...
    checkFormats();
    if (editorRef.current) {
       updateCurrentSnippet({ text: editorRef.current.innerHTML });
    }
  };
  ...
  const handleInsertLink = () => {
    ...
    // Add class for cursor pointer and blue color
  ...
  const onEmojiClick = (emojiData: any) => {
     // Intentionally do NOT close the popover (setIsEmojiPopoverOpen(false))
     // This allows the user to insert multiple emojis just like WhatsApp
        ...
        <div className="flex h-full overflow-hidden">
          ...
          <div className="w-[300px] flex-shrink-0 border-r border-gray-100 dark:border-gray-800 flex flex-col bg-gray-50/50 dark:bg-gray-900/50">
            <div className="p-6 border-b border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 sticky top-0 z-10">
              <DialogTitle className="text-xl font-bold mb-1">Text Snippets</DialogTitle>
              <DialogDescription className="text-gray-500 text-sm">
                Add plain text content to train your AI.
              </DialogDescription>
            </div>
                ...
                >
                  <div className="flex items-center gap-3 overflow-hidden">
                    <div className={`
                       ...
                       ${selectedId === snippet.id ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-500'}
                    `}>
                    ...
                    <div className="flex flex-col min-w-0">
                      ...
                      <span className="text-xs text-gray-400 truncate">
                        {snippet.text.replace(/<[^>]+>/g, '').substring(0, 30) || 'No content'}
                      </span>
                    </div>
                  </div>
             ...
             {selectedId ? (
                <>
                  ...
                  <div className="flex-1 p-6 overflow-y-auto cursor-text" onClick={handleEditorClick}>
                    <div
                      ...
                      onInput={(e) => updateCurrentSnippet({ text: e.currentTarget.innerHTML })}
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-gray-400">
                   <div className="w-16 h-16 bg-gray-50 dark:bg-gray-800 rounded-full flex items-center justify-center mb-4">
                      <div className="w-8 h-8" style={{ '--stroke-0': 'currentColor' } as React.CSSProperties}>
                        <Vector118920 />
                      </div>
                   </div>
...
const TextStyleButton = ({ onClick, label, preview, icon, iconColor }: { onClick: () => void, label: string, preview: React.ReactNode, icon: React.ReactNode, iconColor?: string }) => (
  ...
  >
    <div className="w-6 flex items-center justify-center text-black dark:text-black font-bold">
      {icon}
    </div>
