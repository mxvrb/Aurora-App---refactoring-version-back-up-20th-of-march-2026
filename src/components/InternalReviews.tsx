...
import { ChevronLeft, ClipboardList, Info, Brain, Search } from 'lucide-react';
...
export function InternalReviews({ onBack, onSave }: InternalReviewsProps) {
  ...
  const [initialState] = useState(() => {
    ...
    try {
      const saved = localStorage.getItem('internalReviewsSettings');
  ...
  const defaultSettings = {
    ...
    messageTemplate: "Hi {name}, glad you came by! How was your experience with us?",
  ...
  const [storageEnabled, setStorageEnabled] = useState(savedSettings.storageEnabled ?? true);

  const [isGeneratingMessage, setIsGeneratingMessage] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');
  const [filterBy, setFilterBy] = useState<'all' | 'name' | 'date'>('all');

  const [storedReviews] = useState<StoredReview[]>([
    { id: '1', customerName: 'John Doe', review: 'Great service! Very professional.', date: '2024-11-20' },
    { id: '2', customerName: 'Jane Smith', review: 'Loved the experience, will come again!', date: '2024-11-22' },
  ]);

  const handleGenerateMessage = async () => {
    if (isGeneratingMessage) return;
    setIsGeneratingMessage(true);
    toast.info('Generating AI feedback message...');
    try {
      const { companyName, lineOfBusiness } = getBusinessContext();
      const systemPrompt = `You are a business messaging assistant. Generate a short, friendly internal feedback request message. Use {name} as a placeholder. Keep it under 2 sentences. Warm and professional tone.`;
      const userPrompt = `Generate an internal feedback/review request message${companyName ? ` for ${companyName}` : ''}${lineOfBusiness ? ` (${lineOfBusiness})` : ''}. Use {name} as a placeholder.`;
      const generated = await generateWithOpenAI(systemPrompt, userPrompt);
      setMessageTemplate(generated);
      toast.success('AI feedback message generated!');
    } catch (error: any) {
  ...
  useEffect(() => {
    if (!isMounted) return;
    const settings = {
      isEnabled, startTiming, customStartValue, customStartUnit, stopAfterEnabled, stopAfterValue, stopAfterUnit,
      oneReviewPerCustomer, messageTemplate, enableReminder, reminderDays,
      timeframeEnabled, distributionEnabled, messageEnabled, reminderEnabled, storageEnabled,
    };
    localStorage.setItem('internalReviewsSettings', JSON.stringify(settings));
  ...
  }, [isMounted, isEnabled, startTiming, customStartValue, customStartUnit, stopAfterEnabled, stopAfterValue, stopAfterUnit, oneReviewPerCustomer, messageTemplate, enableReminder, reminderDays, timeframeEnabled, distributionEnabled, messageEnabled, reminderEnabled, storageEnabled]);

  const hasUnsavedChanges = () => {
    const cur = { isEnabled, startTiming, customStartValue, customStartUnit, stopAfterEnabled, stopAfterValue, stopAfterUnit, oneReviewPerCustomer, messageTemplate, enableReminder, reminderDays, timeframeEnabled, distributionEnabled, messageEnabled, reminderEnabled, storageEnabled };
    const sav = { isEnabled: savedSettings.isEnabled, startTiming: savedSettings.startTiming, customStartValue: savedSettings.customStartValue ?? '14', customStartUnit: savedSettings.customStartUnit ?? 'days', stopAfterEnabled: savedSettings.stopAfterEnabled, stopAfterValue: savedSettings.stopAfterValue, stopAfterUnit: savedSettings.stopAfterUnit, oneReviewPerCustomer: savedSettings.oneReviewPerCustomer, messageTemplate: savedSettings.messageTemplate, enableReminder: savedSettings.enableReminder, reminderDays: savedSettings.reminderDays, timeframeEnabled: savedSettings.timeframeEnabled, distributionEnabled: savedSettings.distributionEnabled, messageEnabled: savedSettings.messageEnabled, reminderEnabled: savedSettings.reminderEnabled, storageEnabled: savedSettings.storageEnabled };
    return JSON.stringify(cur) !== JSON.stringify(sav);
  };

  const handleBack = () => { if (hasUnsavedChanges()) { setShowUnsavedChanges(true); } else { onBack(); } };
  const handleDiscard = () => { if (initialState) { localStorage.setItem('internalReviewsSettings', JSON.stringify(initialState)); } setShowUnsavedChanges(false); onBack(); };
  ...
  const filteredReviews = storedReviews.filter(review => {
    ...
    if (filterBy === 'name') return review.customerName.toLowerCase().includes(query);
    else if (filterBy === 'date') return review.date.includes(query);
    else return review.customerName.toLowerCase().includes(query) || review.review.toLowerCase().includes(query) || review.date.includes(query);
  });
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-start gap-3 flex-1 min-w-0">
                <div className={`flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center transition-colors duration-300 ${isEnabled ? 'bg-blue-100 dark:bg-blue-900/50' : 'bg-gray-100 dark:bg-gray-700'}`}>
                  <ClipboardList className={`w-5 h-5 transition-colors duration-300 ${isEnabled ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400 dark:text-gray-500'}`} />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    <span className={`text-sm font-medium transition-colors duration-300 ${isEnabled ? 'text-gray-900 dark:text-gray-100' : 'text-gray-500 dark:text-gray-400'}`}>Activate Internal Reviews</span>
                    <div className="relative group" onClick={(e) => e.stopPropagation()}>
                      ...
                      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 bg-gray-900 dark:bg-gray-700 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap z-50 shadow-lg">
                        Collect and store private feedback from customers.
                  ...
                  <p className={`text-xs mt-0.5 transition-colors duration-300 ${isEnabled ? 'text-gray-600 dark:text-gray-400' : 'text-gray-400 dark:text-gray-500'}`}>Private feedback stored within the AI system</p>
                </div>
              </div>
            ...
            <motion.div initial={false} animate={{ opacity: isEnabled ? 1 : 0.4 }} transition={{ duration: 0.45, ease: [0.23, 1, 0.32, 1] }}>
              <div className="space-y-8 transition-colors duration-300 pt-2" style={{ pointerEvents: isEnabled ? 'auto' : 'none' }}>
                ...
                <div className="space-y-8 pt-4 pb-2">
                        ...
                        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.45, ease: [0.23, 1, 0.32, 1] }} className="overflow-hidden">
                          <div className="space-y-4 pt-2 ml-2 mt-2">
                            <div className="border-l-2 border-blue-400 dark:border-blue-600 pl-4 space-y-4">
                              <div className="space-y-2">
                                <Label className="text-sm text-gray-700 dark:text-gray-300">Start asking for feedback</Label>
                              ...
                              <div className="flex items-center justify-between p-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl relative">
                                <div>
                                  ...
                                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Limit feedback requests to a specific timeframe</p>
                                </div>
                  ...
                  <div className="space-y-2 relative">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-gray-600 dark:text-gray-400 mt-0.5">Prevent spamming customers</p>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="text-sm text-gray-600 dark:text-gray-400">
                            ...
                            <div className="border-l-2 border-blue-400 dark:border-blue-600 pl-4 space-y-4">
                              <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl relative">
                                <div className="pr-4">
                                  ...
                                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 leading-relaxed">Don't ask repeat customers for feedback again if they've already been asked previously.</p>
                                </div>
                      ...
                      {messageEnabled && (
                        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.45, ease: [0.23, 1, 0.32, 1] }} className="overflow-hidden">
                          <div className="border-l-2 border-blue-400 dark:border-blue-600 pl-4 space-y-2 pt-2 ml-2 mt-2 py-1">
                            ...
                            <Textarea value={messageTemplate} onChange={(e) => setMessageTemplate(e.target.value)} rows={3} placeholder="Hi {name}, glad you came by! How was your experience with us?" className="text-sm rounded-xl" />
                            ...
                            <p className="text-xs text-gray-500 dark:text-gray-400">Use {'{name}'} as a placeholder</p>
                          </div>
                        </motion.div>
                      )}
                    </AnimatePresence>
                  ...
                  {/* Stored Reviews Section */}
                  <div className="space-y-2 relative pt-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <Label className="text-gray-900 dark:text-gray-100">Stored Reviews ({storedReviews.length})</Label>
                        <p className="text-xs text-gray-600 dark:text-gray-400 mt-0.5">Search and manage internal feedback</p>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="text-sm text-gray-600 dark:text-gray-400">
                          {storageEnabled ? 'Storage On' : 'Storage Off'}
                        </span>
                        <Switch checked={storageEnabled} onCheckedChange={setStorageEnabled} className="data-[state=checked]:bg-blue-600" />
                      </div>
                    </div>
                    <AnimatePresence>
                      {storageEnabled && (
                        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.45, ease: [0.23, 1, 0.32, 1] }} className="overflow-hidden">
                          <div className="border-l-2 border-blue-400 dark:border-blue-600 pl-4 space-y-3 ml-2 mt-3 py-1">
                            <div className="flex items-center gap-2">
                              <div className="relative flex-1">
                                <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400" />
                                <Input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search reviews..." className="pl-10 text-sm" />
                              </div>
                              <Select value={filterBy} onValueChange={(value) => setFilterBy(value as 'all' | 'name' | 'date')}>
                                <SelectTrigger className="w-32"><SelectValue /></SelectTrigger>
                                <SelectContent>
                            ...
                            <div className="space-y-2 max-h-96 overflow-y-auto pt-2">
                              {filteredReviews.length === 0 ? (
                                <p className="text-sm text-gray-500 dark:text-gray-400 py-8 text-center">No reviews found</p>
                              ) : (
                                filteredReviews.map((review) => (
                                  <div key={review.id} className="p-4 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl relative">
                                    <div className="flex items-start justify-between mb-2">
                                      <div>
                                        <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{review.customerName}</p>
